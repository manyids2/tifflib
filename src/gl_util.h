...
static void vertex_buffer_create(GLuint *obj, GLenum target, void *data,
                                 size_t size);
static void vertex_array_pointer(const char *attr, GLint size, GLenum type,
                                 GLboolean norm, size_t stride, size_t offset);
...
static void index_buffer_add(index_buffer *ib, const uint *data, uint count,
                             uint addend);
static void index_buffer_add_primitves(index_buffer *ib, primitive_type type,
                                       uint count, uint addend);
...
static uint vertex_buffer_count(vertex_buffer *vb) { return vb->count; }

static void *vertex_buffer_data(vertex_buffer *vb) { return vb->data; }
...
static uint vertex_buffer_add(vertex_buffer *vb, vertex v) {
  if (vb->count >= vb->total) {
    vb->total <<= 1;
    vb->data = (vertex *)realloc(vb->data, sizeof(vertex) * vb->total);
  }
  uint idx = vb->count++;
  vb->data[idx] = v;
  return idx;
...
static void vertex_buffer_dump(vertex_buffer *vb) {
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    printf("  [%7zu] = { "
           ...
           i, v->pos.vec[0], v->pos.vec[1], v->pos.vec[2], v->norm.vec[0],
           v->norm.vec[1], v->norm.vec[2], v->uv.vec[0], v->uv.vec[1],
...
static uint index_buffer_count(index_buffer *ib) { return ib->count; }

static void *index_buffer_data(index_buffer *ib) { return ib->data; }
...
static void index_buffer_add(index_buffer *ib, const uint *data, uint count,
  ...
  if (ib->count + count >= ib->total) {
    ...
    ib->data = (uint *)realloc(ib->data, sizeof(uint) * ib->total);
  }
...
static buffer load_file(const char *filename) {
  ...
  if ((f = fopen(filename, "rb")) == NULL) {
    printf("gears_create_shader_from_file: open: %s: %s", filename,
           strerror(errno));
    exit(1);
  }
  if (fstat(fileno(f), &statbuf) < 0) {
    printf("gears_create_shader_from_file: stat: %s: %s", filename,
           strerror(errno));
    exit(1);
  }
...
typedef void (*func_4_1_glShaderBinary)(GLsizei, const GLuint *, GLenum,
                                        const void *, GLsizei);
typedef void (*func_4_3_glGetProgramResourceName)(GLuint, GLenum, GLuint,
                                                  GLsizei, GLsizei *, GLchar *);
typedef void (*func_4_6_glSpecializeShader)(GLuint, const GLchar *, GLuint,
...
static void muglInit() {
  ...
  muglShaderBinary =
      (func_4_1_glShaderBinary)muglGetProcAddress("glShaderBinary");
  muglGetProgramResourceName =
      (func_4_3_glGetProgramResourceName)muglGetProcAddress(
          "glGetProgramResourceName");
  muglSpecializeShader =
      (func_4_6_glSpecializeShader)muglGetProcAddress("glSpecializeShader");
...
static GLuint compile_shader(GLenum type, const char *filename) {
  ...
  if (is_spirv) {
    ...
    glShaderSource(shader, (GLsizei)1, (const GLchar *const *)&buf.data,
                   &length);
...
static void reflect_gl2(GLuint program, GLint *numattrs, GLint *numuniforms) {
  ...
  for (GLint i = 0; i < *numattrs; i++) {
    char namebuf[128];
    muglGetProgramResourceName(program, GL_PROGRAM_INPUT, i, sizeof(namebuf),
                               NULL, namebuf);
  ...
  for (GLint i = 0; i < *numuniforms; i++) {
    char namebuf[128];
    muglGetProgramResourceName(program, GL_UNIFORM, i, sizeof(namebuf), NULL,
                               namebuf);
...
static void reflect_gl4(GLuint program, GLint *numattrs, GLint *numuniforms) {
  ...
  for (GLint i = 0; i < *numattrs; i++) {
    ...
    GLenum type = GL_ZERO;
    char namebuf[128];
    glGetActiveAttrib(program, i, sizeof(namebuf) - 1, &namelen, &size, &type,
                      namebuf);
    namebuf[namelen] = 0;
    attr_list_set(&attrs, namebuf, i);
  }
  ...
  for (GLint i = 0; i < *numuniforms; i++) {
    ...
    GLenum type = GL_ZERO;
    char namebuf[128];
    glGetActiveUniform(program, i, sizeof(namebuf) - 1, &namelen, &size, &type,
                       namebuf);
    namebuf[namelen] = 0;
    attr_list_set(&uniforms, namebuf, glGetUniformLocation(program, namebuf));
  }
...
static void uniform_3f(const char *uniform, GLfloat v1, GLfloat v2,
                       GLfloat v3) {
  GLuint val;
  if ((val = attr_list_value(&uniforms, uniform)) != ATTR_NOT_FOUND) {
